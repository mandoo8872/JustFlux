...
export const useAnnotationSelection = () => {
  ...
  const selectAnnotation = useCallback((annotationId: string, multiSelect = false) => {
    console.log('🎯 [useAnnotationSelection] Selecting annotation:', annotationId, 'multiSelect:', multiSelect);
  ...
  const clearSelection = useCallback(() => {
    console.log('🎯 [useAnnotationSelection] Clearing selection');
    setSelectionState(prev => ({
      ...prev,
      selectedAnnotationIds: []
    }));
  ...
  const handleAnnotationClick = useCallback((annotationId: string, e: React.MouseEvent) => {
    ...
    if (newClickCount === 2) {
      // 더블클릭: 인라인 편집 시작
      console.log('🎯 [useAnnotationSelection] Double click detected for inline edit');
