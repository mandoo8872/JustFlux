...
export async function loadPdfFile(file: File): Promise<LoadPdfResult> {
  try {
    console.log(`📄 [PDF] Loading file: ${file.name} (${(file.size / 1024).toFixed(1)} KB)`);
    ...
    console.log(`✅ [PDF] Loaded: ${pdfProxy.numPages} pages`);
    ...
    for (let i = 1; i <= pdfProxy.numPages; i++) {
      ...
      if (i % 10 === 0) {
        console.log(`   Processed ${i}/${pdfProxy.numPages} pages...`);
      }
    }
    
    document.pages = pages;
    
    console.log(`✅ [PDF] Document created: ${pages.length} pages`);
...
export async function renderPdfPage(
  ...
  try {
    ...
    console.log(`✅ [PDF] Rendered page ${pageIndex + 1} at ${scale}x scale`);
  } catch (error) {
